...
!
      ...
      use icedrv_domain_size, only: n_algae, n_zaero, n_doc, n_dic, n_don, n_zmp
      use icedrv_domain_size, only: n_fed, n_fep, max_nsw, n_bgc, n_aero, n_mp
      ...
      use icepack_intfc, only: icepack_max_nbtrcr, icepack_max_mp
...
!  Initialize shortwave
      ...
      logical (kind=log_kind) :: tr_brine, tr_zaero, tr_zmp, tr_bgc_N
      ...
      integer (kind=int_kind), dimension(icepack_max_mp) :: nlt_zmp_sw
      ...
      !-----------------------------------------------------------------
         ...
         call icepack_query_tracer_flags(tr_brine_out=tr_brine, &
              tr_zaero_out=tr_zaero, tr_zmp_out=tr_zmp, tr_bgc_N_out=tr_bgc_N)
      ...
      !-----------------------------------------------------------------
        ...
        !-----------------------------------------------------------------
        ! Define microplastics tracer on shortwave grid
        !-----------------------------------------------------------------

               if (dEdd_algae .and. (tr_zmp)) then
                  do n = 1, ncat
                     do k = 1, nbtrcr_sw
                        trcrn_sw(i,k,n) = ztrcr_sw(k,n)
                     enddo
...
!  Initialize vertical profile for biogeochemistry
      ...
      use icedrv_flux, only: sss, nit, amm, sil, dmsp, dms, algalN, &
          doc, don, dic, fed, fep, zaeros, zmps, hum
      ...
      call icepack_query_tracer_sizes(max_nbtrcr_out=max_nbtrcr, &
           ...
           max_dic_out=max_dic, max_aero_out=max_aero, max_fe_out=max_fe, max_mp_out=max_mp)
...
!        Nicole Jeffery, LANL
      ...
      integer (kind=int_kind), dimension(icepack_max_mp) :: &
         nlt_zmp_sw         ! points to microplastics in trcrn_sw
      ...
      integer (kind=int_kind), dimension(icepack_max_aero) :: &
       ...
       integer (kind=int_kind), dimension(icepack_max_mp) :: &
         nt_zmp         , & !  microplastics
         nt_zmp_sw          !  for shortwave calculation
     ...
     real (kind=dbl_kind), dimension(icepack_max_nbtrcr) :: &
      ...
      namelist /zbgc_nml/  &
        tr_brine, tr_zaero, tr_zmp, modal_aero, skl_bgc, &
      ...
      tr_zmp          = .false.  ! z microplastic tracers
      ...
      if (.not. tr_brine) then
         ...
         if (tr_zmp) then
            write(nu_diag,*) 'WARNING: tr_brine = F and tr_zmp = T'
      ...
      !-----------------------------------------------------------------
      if (tr_zmp .and. .not. z_tracers) z_tracers = .true.

      if (n_zmp > icepack_max_mp) then
         print*, 'error:number of z microplastics exceeds icepack_max_mp'
         call icedrv_system_abort(file=__FILE__,line=__LINE__)
      endif
      ...
      if (tr_zmp .and. TRZMP <  1) then
         write (nu_diag,*) ' '
         write (nu_diag,*) 'comp_ice must have number of TRZMP > 0'
         write (nu_diag,*) 'in order to solve z microplastics:',TRZMP
         call icedrv_system_abort(file=__FILE__,line=__LINE__)
      endif
      ...
      ! vectors of size icepack_max_mp
      nlt_zmp(:) = 0
      nlt_zmp_sw(:) = 0
      nt_zmp(:) = 0
      nt_zmp_sw(:) = 0
